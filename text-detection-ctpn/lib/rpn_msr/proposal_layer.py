...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    print("anchors.shape", _anchors.shape)
    ...
    reshape = np.reshape(rpn_cls_prob_reshape, [1, height, width, _num_anchors, 2])
    print("reshape.shape ", reshape.shape)
    scores = np.reshape(reshape[:, :, :, :, 1], [1, height, width, _num_anchors])
    ...
    print("anchors", anchors.shape, "bbox_deltas", bbox_deltas.shape)
