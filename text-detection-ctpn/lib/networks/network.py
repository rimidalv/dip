...
from ..rpn_msr.proposal_layer_tf import proposal_layer_tf
from ..rpn_msr.anchor_target_layer_tf import anchor_target_layer as anchor_target_layer_py
from ..rpn_msr.generate_anchors_tf import generate_anchors_pre_tf
...
class Network(object):
    ...
    @layer
    def proposal_layer_tf(self, input, _feat_stride, anchor_scales, cfg_key, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]

        _im_info = input[2]
        height = tf.to_int32(_im_info[0, 0] / _feat_stride[0])
        width = tf.to_int32(_im_info[0, 1] / _feat_stride[0])
        height = print_tf(height, "#height")
        width = print_tf(width, "#width")
        _anchors, _num_anchors = generate_anchors_pre_tf(height, width, _feat_stride[0])

        _anchors = print_tf(_anchors, "#_anchors")
        _num_anchors = print_tf(_num_anchors, "#_num_anchors")

        with tf.variable_scope(name) as scope:
            blob, bbox_delta = proposal_layer_tf(input[0], input[1], input[2], cfg_key, _anchors, _num_anchors, height,
                                                 width)

            blob = print_tf(blob, "#blob")
            bbox_delta = print_tf(bbox_delta, "#bbox_delta")

            rpn_rois = tf.convert_to_tensor(tf.reshape(blob, [-1, 5]), name='rpn_rois')  # shape is (1 x H x W x A, 2)
            rpn_targets = tf.convert_to_tensor(bbox_delta, name='rpn_targets')  # shape is (1 x H x W x A, 4)
            self.layers['rpn_rois'] = rpn_rois
            self.layers['rpn_targets'] = rpn_targets
            rpn_rois = print_tf(rpn_rois, "#rpn_rois")
            rpn_targets = print_tf(rpn_targets, "#rpn_targets")
            return rpn_rois, rpn_targets
